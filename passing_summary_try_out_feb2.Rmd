...
```{r}
percentile.1st <- function() {
  yards.y <- passing_12_22$yards.y
  variables.pass <- passing_12_22[5:42]
  average.if.1st <- data.frame(tier.1st=numeric())
  
  for (i in variables.pass) {
    averages.1st <- mean(yards.y[i >= quantile(i, zpercentiles[10], na.rm=TRUE) & i <= quantile(i, zpercentiles[9], na.rm=TRUE)], na.rm=TRUE)
    average.if.1st[nrow(average.if.1st)+1,] <- averages.1st
  }
return(data.frame(average.if.1st))
}
percentile.1st()
print(average.if.1st)

```

```{r}
percentile.2nd <- function() {
  yards.y <- passing_12_22$yards.y
  variables.pass <- passing_12_22[5:42]
  average.if.2nd <- data.frame(tier.2nd=numeric())
  
  for (i in variables.pass) {
    averages.2nd <- mean(yards.y[i >= quantile(i, zpercentiles[9], na.rm=TRUE) & i <= quantile(i, zpercentiles[8], na.rm=TRUE)], na.rm=TRUE)
    average.if.2nd[nrow(average.if.2nd)+1,] <- averages.2nd
  }
print(average.if.2nd)
}
percentile.2nd()
...
```{r}
percentile.1st.tier <- c(percentile.1st())
percentile.2nd.tier <- c(percentile.2nd())
newlist <- cbind(percentile.1st.tier, percentile.2nd.tier)
newlist
```
