...
```{r echo=FALSE}
big_association <- rbind(big_23, big_22, big_21)  #vertically combining the three years of all the individual weeks
big_association <- big_association %>% #cleaning the data
  select(-c(player)) %>%
  na.omit()
...
```{r echo=FALSE}
big_trios <- rbind(trios_23, trios_23, trios_21)
big_trios <- big_trios %>% 
  select(-c(player)) %>%
  na.omit() %>%
  round(2)
