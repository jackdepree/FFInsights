...
dvoa_22 <- dvoa_22 %>%
  ...
  separate(col=`Rush DVOA`, into=c('Rush_DVOA', NA), sep="%", convert=TRUE) %>%
  separate(col=`Variance`, into=c('Variance', NA), sep="%", convert=TRUE)
sal_12 <- salary %>% filter(`X2012`=="TRUE")
sal_13 <- salary %>% filter(`X2013`=="TRUE")
sal_14 <- salary %>% filter(`X2014`=="TRUE")
sal_15 <- salary %>% filter(`X2015`=="TRUE")
sal_16 <- salary %>% filter(`X2016`=="TRUE")
sal_17 <- salary %>% filter(`X2017`=="TRUE")
sal_18 <- salary %>% filter(`X2018`=="TRUE")
sal_19 <- salary %>% filter(`X2019`=="TRUE")
sal_20 <- salary %>% filter(`X2020`=="TRUE")
sal_21 <- salary %>% filter(`X2021`=="TRUE")
sal_22 <- salary %>% filter(`X2022`=="TRUE")
```
...
rb_12_22 <- rb_12_22 %>% select(c(player, player_game_count.x:fantasy_points.x, yards_rush.y, yards_rec.y, 
                                ...
                                -c(rec_yards.x, franchise_id.x, Year.Signed.x:Year.End.x, Position.x, Team.x))
glimpse(rb_12_22)
```
...
expectation <- rb_12_22 %>% select(c(fantasy_points.y, ADP.x, Pick.x, Appearances.x, AllAmerican.x, APY.x, Guaranteed.x, APY..Of.Cap.x,
                                     Inflated.Value.x, Inflated.APY.x, Inflated.Guaranteed.x, Contract.Years.x))
```
