...
Plotting
```{r}
cor(passing_12_22$yards.y, passing_12_22[7], use="complete.obs")
```
...
cor.60 <- colnames(passing_12_22[which(yards.cor>=.6)+4])
cor.60
...
More Plotting
```{r}
plot.default(average.1st, ylab="Yards", col="red", cex=2.5, lwd=2)
points(average.3rd, col="darkorange", cex=2, lwd=2)
points(average.5th, col="darkgreen", cex=1.5, lwd=2)
points(average.7th, col="blue", cex=1, lwd=2)
points(average.10th, col="black", cex=.75, lwd=2)
```
...
player.implied <- data.frame()

if (pass_21[1, 5] >= quantile(pass_21[,5], zpercentiles[1], na.rm=TRUE)) {
  transmutate_at(player.implied, player=pass_21[1,5], game_count.x=average.if[1,11])
  player.implied
