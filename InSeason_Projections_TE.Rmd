...
Calculating the correlation of each variable to both the current week's fantasy points and the year's total fantasy points.
```{r}
...
comatrix <- comatrix %>% 
  ...
  mutate(cor.diff=round(cor.x-cor.y,3)) %>%
  mutate(coh.diff=round(coh.x-coh.y,3)) %>%
  mutate(diff=round(effect.x-effect.y,3))
comatrix
```
