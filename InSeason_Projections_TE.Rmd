...
#Data Analysis
```{r}
cor.x <- cor(big_association, big_association$fantasyPts.x, use="complete.obs")
cor.y <- cor(big_association, big_association$fantasyPts.y, use="complete.obs")
significant.x <- which(cor.x>.1)
significant.y <- which(cor.y>.1)
```

```{r}
apply(big_association[,significant.x], 2, mean)
```
