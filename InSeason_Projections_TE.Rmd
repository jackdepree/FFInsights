...
var.x.stable.index <- which(var.x<2)
var.x.unstable.index <- which(var.x>2)
var.y.stable.index <- which(var.y<2)
var.y.unstable.index <- which(var.y>2)
head(var.x, 10)
```
...
ggplot(big_association, aes(x=recTd40s)) +
  ...
  labs(title="Distribution of Receiving Touchdowns >= 40 Yards (Stable)", x="Receiving TDs >= 40yds per Game", y="")

ggplot(big_association, aes(x=recRec40s)) +
  geom_histogram(binwidth=sd(big_association$recRec40s)/2, fill="darkorchid4") +
  labs(title="Distribution of Receiving Receptions >= 40 Yards (Stable)", x="Receiving Recs >= 40yds per Game", y="")
      
ggplot(big_association, aes(x=recTarg)) +
  geom_histogram(binwidth=sd(big_association$recTarg)/2, fill="darkorchid4") +
  labs(title="Distribution of Receiving Targets (Unstable)", x="Receiving Targets per Game", y="")

ggplot(big_association, aes(x=rzRecTdPct)) +
  geom_histogram(binwidth=sd(big_association$rzRecTdPct)/2, fill="darkorchid4") +
  labs(title="Distribution of rzRecTdPct (Unstable)", x="Receiving rzRecTdPct per Game", y="")
```
...
modeller <- function(dat, indep, dep) {
  ...
  data <- dat %>% select(c(indep, dep)) %>% na.omit()
  intercept_only <- lm(dep ~ 1, data=data)
  all <- lm(dep ~ ., data=data)
