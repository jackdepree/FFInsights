...
weeks_pair1_23 <- rbind(wk1_23, wk2_23) %>%           #binds the two dfs vertically into one df
  ...
  filter(n() > 1) %>%                                 #filters out players who only appear once, meaning that they only played one game
...
ggplot(big_association, aes(x=catch, y=fantasyPts.x)) +
  geom_smooth(data=subset(big_association, recTarg>1), method="loess", se=F, color="violet", size=1.25) +
  geom_smooth(data=subset(big_association, recTarg>3), method="loess", se=F, color="darkorchid2", size=1.25) +
  geom_smooth(data=subset(big_association, recTarg>5), method="loess", se=F, color="darkorchid3", size=1.25) +
  geom_smooth(data=subset(big_association, recTarg>7), method="loess", se=F, color="darkorchid4", size=1.25) +
...
ggplot(big_pairs, aes(x=recTarg, y=fantasyPts.y)) +
  geom_smooth(data=subset(big_pairs, recTds<.5), method="loess", se=F, color="darkorchid2", size=1.25) +
  geom_smooth(data=subset(big_pairs, recTds>.5), method="loess", se=F, color="darkorchid4", size=1.25) +
